...
export default function ManageLocationsPage() {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      // ✅ Backend akan otomatis filter berdasarkan role:
      // - Admin: semua lokasi
      // - Mitra: hanya lokasi milik sendiri
      const { data } = await api.get('/locations', { params }); 
      
      console.log('Response received:');
      console.log('- Total locations:', data.total);
      console.log('- Count:', data.count);
      console.log('- First location owner:', data.data[0]?.owner);
      
      setLocations(data.data);
      console.log('✅ State updated. Locations in state:', data.data);
    } catch (error: any) {
  ...
  const handleCreateSuccess = () => {
    ...
    toast.success(
      isMitra 
        ? 'Lokasi baru berhasil ditambahkan! Status: Menunggu persetujuan admin'
  ...
  const handleEditSuccess = () => {
    ...
    toast.success(
      isMitra 
        ? 'Lokasi berhasil diupdate! Status: Menunggu persetujuan admin'
      ...
      {isMitra && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p>
              Lokasi yang Anda tambahkan akan melalui proses verifikasi oleh admin. 
              ...
              {rejectedCount > 0 && <span className="block mt-1">Anda dapat mengedit dan mengajukan kembali lokasi yang ditolak.</span>}
            </p>
          </div>
        </div>
      )}
